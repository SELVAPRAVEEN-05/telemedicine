...
type RecordDetailsNavigationProp =
...
export default function RecordDetailsScreen({ route, navigation }: Props) {
  ...
  const hospitalRecords = currentPatient
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.notescomponent}>
          <Text style={styles.sectionTitle}>
            {currentPatient
              ? `${currentPatient.name}'s Records at ${record.title}`
              : 'Medical Records'}
          </Text>
          {hospitalRecords.length > 0 ? (
            hospitalRecords.map(recordItem => (
              ...
              >
                ...
                {recordItem.details.medications &&
                  recordItem.details.medications.length > 0 && (
                    <Text style={styles.medicationsCount}>
                      Medications: {recordItem.details.medications.length}{' '}
                      prescribed
                    </Text>
                  )}
